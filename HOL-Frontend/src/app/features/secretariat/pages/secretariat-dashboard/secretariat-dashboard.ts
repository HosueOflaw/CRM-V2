...
@Component({
  ...
  imports: [DashboardLayout, FormsModule, CommonModule, ToastModule],
...
export class SecretariatDashboard {
  ...
  editAutoNumber() {
    ...
    if (isNaN(id)) {
      this.messageService.add({ severity: 'error', summary: 'خطأ', detail: 'الكود يجب أن يكون رقماً' });
      return;
    }
    ...
    this.dataViewService.getAutoNumberById(code).subscribe({
      next: (data) => {
        if (data) {
          ...
          this.messageService.add({ severity: 'info', summary: 'تم العثور', detail: 'تم العثور على الرقم الآلي' });
        } else {
          this.editFormData.autoNumber = '';
          this.currentAutoNumberObject = {};
          this.messageService.add({ severity: 'warn', summary: 'غير موجود', detail: 'لم يتم العثور على رقم آلي بهذا الكود' });
        }
      },
      error: (err) => {
        console.error(err);
        this.editFormData.autoNumber = '';
        this.currentAutoNumberObject = {};
        this.messageService.add({ severity: 'error', summary: 'خطأ', detail: 'حدث خطأ أثناء الحلب' });
      }
    });
  }
