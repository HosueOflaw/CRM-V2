...
export class ImportFileDetailsPage implements OnInit, OnDestroy {
    ...
    uploadFile() {
        ...
        this.uploading = true;
        this.importService.uploadFileDetails(this.selectedFile).subscribe({
            next: (res) => {
                ...
                this.swal.toast({ icon: 'info', title: 'بدأت المعالجة', text: 'يتم الآن رفع ومعالجة ملف تفاصيل الملفات في الخلفية' });
            },
            error: (err) => {
                this.uploading = false;
    ...
    listenToProgress() {
        this.signalr.message$.pipe(takeUntil(this.destroy$)).subscribe(msg => {
            if (msg.type === 'excel_import_progress') {
                ...
                // Refresh list on any completion
                this.loadJobs();
            }
        });
    }
