...
@Component({
    ...
    template: `
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8 flex justify-center">
      <!-- Summary Cards -->
      ...
      <p-card styleClass="shadow-sm border-0 bg-green-50 dark:bg-green-900/10">
        <div class="flex flex-col">
          <span class="text-sm text-gray-500 mb-1">عمليات ناجحة</span>
          <span class="text-2xl font-bold text-green-600" *ngIf="!loading">{{ stats?.summary?.successfulJobs }}</span>
          <p-skeleton width="3rem" height="2rem" *ngIf="loading"></p-skeleton>
        </div>
      </p-card>
      ...
      <p-card styleClass="shadow-sm border-0 bg-red-50 dark:bg-red-900/10">
        <div class="flex flex-col">
          <span class="text-sm text-gray-500 mb-1">عمليات فشلت</span>
          <span class="text-2xl font-bold text-red-600" *ngIf="!loading">{{ stats?.summary?.failedJobs }}</span>
          <p-skeleton width="3rem" height="2rem" *ngIf="loading"></p-skeleton>
        </div>
      </p-card>
      ...
      <p-card styleClass="shadow-sm border-0 bg-purple-50 dark:bg-purple-900/10">
        <div class="flex flex-col">
          <span class="text-sm text-gray-500 mb-1">إجمالي السجلات</span>
          <span class="text-2xl font-bold text-purple-600" *ngIf="!loading">{{ stats?.summary?.totalRowsProcessed?.toLocaleString() }}</span>
          <p-skeleton width="3rem" height="2rem" *ngIf="loading"></p-skeleton>
        </div>
      </p-card>
    </div>
